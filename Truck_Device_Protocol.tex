...
TOF Sensor 1     & I2C (e.g.\ VL53L1X)   & Cargo front rangefinder; fill level, 0.0-4.0\,m\\
TOF Sensor 2     & I2C (e.g.\ VL53L1X)   & Cargo rear rangefinder; fill level, 0.0-4.0\,m\\
\bottomrule
\end{tabular}
\end{center}
...
Broker host     & \texttt{140.150.152.166} (POC) / \texttt{mqtt.siam-ev.example.com}\\
...
Method          & POST\\
URL             & \texttt{http://140.150.152.166:3000/v1/trucks/\{id\}/photos} (POC)\\
...
% ==============================================================================
\section{Telemetry Message --- GPS, OBD-II, and Cargo TOF}
% ==============================================================================
...
\begin{lstlisting}[language=json, caption=Telemetry message payload]
{
  ...
  "cargo": {
    "frontDistM": 0.42,        // front cargo TOF sensor, metres, range 0.0-4.0
    "rearDistM":  2.81         // rear cargo TOF sensor, metres, range 0.0-4.0
  }
}
\end{lstlisting}
...
\begin{itemize}
    ...
    \item \texttt{cargo.frontDistM} and \texttt{cargo.rearDistM} are read from
          the front and rear time-of-flight rangefinders mounted at the top of
          the cargo bay. Each reports the distance to the nearest load surface
          in metres, clamped to the sensor's \texttt{0.0-4.0} range; a reading
          near \texttt{4.0} indicates an empty bay and near \texttt{0.0} a full one.
          A sensor fault (no echo) is reported as \texttt{4.0} rather than omitted,
          since cargo fill level is safety- and billing-relevant and must not be
          silently dropped like an unsupported OBD PID.
\end{itemize}
...
POST http://140.150.152.166:3000/v1/trucks/truck-bkk-007/photos
...
\begin{lstlisting}[language=json, caption=Topic: trucks/truck-bkk-007/cmd | QoS 1]
{
  ...
  "payload":{
    "version": "1.5.0",
    "url":     "http://140.150.152.166:3000/firmware/tmu-1.5.0.bin",
    "sha256":  "a3f5e2...c9d1"
  }
}
\end{lstlisting}
