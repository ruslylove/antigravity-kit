...
\node[class] (station) {\textbf{Station}
  \nodepart{second}\ttfamily id\\name\\lat, lng\\type};
\node[class, right=of station] (route) {\textbf{Route}
  \nodepart{second}\ttfamily id\\name\\waypoints[].stationId};
\node[class, below=of station] (truck) {\textbf{Truck}
  \nodepart{second}\ttfamily id\\plateNumber\\status\\currentTripId};
\node[class, below=of route] (trip) {\textbf{Trip}
  \nodepart{second}\ttfamily id\\truckId\\routeId\\status\\events[].stationId};
\node[class, below=of truck] (alert) {\textbf{Alert}
  \nodepart{second}\ttfamily alertId\\sourceType\\sourceId\\severity};
\node[ext, below=of trip] (zone) {\textit{Zone (external)}
  \nodepart{second}\scriptsize owned by EMS;\\opaque \texttt{sourceId}};

\draw[rel] (route.west) -- (station.east)
    node[lbl, above, pos=0.5] {waypoints 1..*};
\draw[rel] (trip.north) -- (route.south)
    node[lbl, right, pos=0.5] {routeId};
\draw[rel] (trip.west) to[bend left=15]
    node[lbl, above] {truckId} (truck.east);
\draw[drel] (truck.east) to[bend right=15]
    node[lbl, below] {currentTripId} (trip.west);
\draw[rel] (alert.north) -- (truck.south)
    node[lbl, right, pos=0.5] {sourceType=Truck};
\draw[drel] (alert.east) -- (zone.west)
    node[lbl, above, pos=0.5] {sourceType=Zone};

\end{tikzpicture}
\caption{Cargo object model --- solid arrows are foreign-key references within
this API (\texttt{Trip.events[].stationId} also references Station, and
\texttt{Trip.routeId}/\texttt{truckId} are the two IDs a Trip is created
with). The dashed box marks data whose owning system is out of scope for this
document (see Section~\ref{sec:integration-notes}).}
\end{figure}
