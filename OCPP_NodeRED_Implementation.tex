...
\subsection{Initializing the Station State}
...
global.set("ocpp_stations", [
  {
    ...
    powerOutput: 0,
    powerLimit: 120,
    schedule: { startTime: "18:00", endTime: "08:00", enabled: true }
  },
  {
    id: "station-bkk-002",
    ...
    powerOutput: 68.4,
    powerLimit: 150,
    schedule: { startTime: "19:00", endTime: "07:00", enabled: false }
  }
]);
return msg;
\end{lstlisting}

\subsection{REST API: Get Station Data}
To allow the UI to fetch the latest telemetry:
\begin{itemize}
    \item \textbf{HTTP In Node}: Method \texttt{GET}, URL \texttt{/api/ocpp/stations}
\begin{lstlisting}
// Merge with persistent schedules (e.g., from schedules.json)
msg.payload = global.get("ocpp_stations").map(s => ({
  ...s,
  schedule: global.get(`schedule_${s.id}`) || s.schedule
}));
return msg;
\end{lstlisting}
...
\subsection{REST API: Remote Stop Command}
...
\subsection{REST API: Charging Schedule}
To set and persist the charging schedule for a specific station:
\begin{itemize}
    \item \textbf{HTTP In Node}: Method \texttt{POST}, URL \texttt{/api/ocpp/stations}
    \item \textbf{Function Node}: Save the schedule to the persistent storage (mocked with global context or file).
\begin{lstlisting}[caption=Handle Charging Schedule Update]
const { stationId, schedule } = msg.payload;

if (!stationId || !schedule) {
    msg.payload = { error: "Missing stationId or schedule" };
    msg.statusCode = 400;
    return msg;
}

// Persist the schedule state
global.set(`schedule_${stationId}`, schedule);

// Update local memory
let stations = global.get("ocpp_stations") || [];
const targetStation = stations.find(s => s.id === stationId);
if (targetStation) {
    targetStation.schedule = schedule;
    global.set("ocpp_stations", stations);
}

msg.payload = { success: true, schedule };
return msg;
\end{lstlisting}
    \item \textbf{HTTP Response Node}: Returns the success response.
\end{itemize}

\subsection{Incoming OCPP: Handling MeterValues}
...
const handleUpdateSchedule = async (stationId, schedule) => {
    try {
        const response = await fetch("http://localhost:1880/api/ocpp/stations", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
