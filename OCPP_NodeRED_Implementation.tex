...
When a physical charger sends a \texttt{MeterValues} message, Node-RED must intercept this payload and update the global \texttt{ocpp\_stations} store so the UI reflects the live energy, power metrics, and capacity (\texttt{powerLimit}) immediately.
